...
function WeekView({
  ...
  const countByDateKey = todos.reduce((accumulator, todo) => {
    accumulator[todo.date] = (accumulator[todo.date] || 0) + 1;
    return accumulator;
  }, {});

  const goPrevWeek = () => onChangeWeek(formatDateKey(addDays(weekStart, -7)));
  const goNextWeek = () => onChangeWeek(formatDateKey(addDays(weekStart, 7)));
  const goCurrentWeek = () =>
